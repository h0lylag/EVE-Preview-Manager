...
impl SourcesTab {
    pub fn ui(&mut self, ui: &mut Ui, profile: &mut crate::config::profile::Profile) -> bool {
        ...
        ui.group(|ui| {
            ...
            ui.label(egui::RichText::new("Manage and edit your custom sources.").weak().small());
            ui.add_space(5.0);
                ...
                for (idx, rule) in profile.custom_windows.iter_mut().enumerate() {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if !is_expanded {
                            ...
                            if let Some(c) = &rule.class_pattern { details.push(format!("Class: {}", c)); }
                            if let Some(t) = &rule.title_pattern { details.push(format!("Title: {}", t)); }
                            ...
                            ui.add_space(4.0);
                             egui::Grid::new(format!("grid_edit_rule_{}", idx))
                                ...
                                .show(ui, |ui| {
                                    ...
                                    let mut class_text = rule.class_pattern.clone().unwrap_or_default();
                                    if ui.text_edit_singleline(&mut class_text).changed() {
                                        rule.class_pattern = if class_text.is_empty() { None } else { Some(class_text) };
                                        changed = true;
                                    }
                                    ui.end_row();
                                    ...
                                    let mut title_text = rule.title_pattern.clone().unwrap_or_default();
                                    if ui.text_edit_singleline(&mut title_text).changed() {
                                        rule.title_pattern = if title_text.is_empty() { None } else { Some(title_text) };
                                        changed = true;
                                    }
                                    ui.end_row();
                                    ...
                                    ui.vertical(|ui| {
                                        ...
                                        let current_ratio = rule.default_width as f32 / rule.default_height as f32;
                                        let detected_preset = {
                                        ...
                                        let id = ui.make_persistent_id(format!("src_ratio_mode_{}", idx));
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            egui::ComboBox::from_id_salt(format!("src_ratio_combo_{}", idx))
                                                ...
                                                .show_ui(ui, |ui| {
                                                    for (name, ratio) in &aspect_ratios {
                                                        if ui.selectable_value(&mut selected_mode, name.to_string(), *name).changed() {
                                                            mode_changed = true;
                                                            if *ratio > 0.0 {
                                                                rule.default_height = (rule.default_width as f32 / ratio).round() as u16;
                                                                changed = true;
                                                            }
                                                        }
                                                    }
                                        ...
                                        ui.horizontal(|ui| {
                                            ui.label("Width:");
                                            if ui.add(egui::Slider::new(&mut rule.default_width, 100..=1200).suffix(" px")).changed() {
                                                changed = true;
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            if is_custom {
                                                if ui.add(egui::Slider::new(&mut rule.default_height, 100..=1200).suffix(" px")).changed() {
                                                    changed = true;
                                                }
                                            } else {
                                                ui.add_enabled(false, egui::Slider::new(&mut rule.default_height, 100..=1200).suffix(" px"));
                                            }
                                        });
                                        
                                        ui.weak(format!("Preview: {}x{}", rule.default_width, rule.default_height));
                                    });
                                    ui.end_row();
